...
在“系统设置 → 隐私与安全性 → 辅助功能”中启用 `OpenPE Hotkey`，然后运行：
...
| `CODESIGN_IDENTITY` | `-` | macOS | macOS 签名身份 |
...
| macOS 只响提示音 | 确认辅助功能权限、Codex 在前台且已选中文字 |
...
Enable `OpenPE Hotkey` in System Settings → Privacy & Security → Accessibility, then run:
...
| `CODESIGN_IDENTITY` | `-` | macOS | macOS signing identity |
...
| macOS only plays a beep | Confirm Accessibility permission, Codex focus, and a non-empty selection |
