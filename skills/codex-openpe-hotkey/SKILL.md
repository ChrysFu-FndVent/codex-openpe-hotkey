...
Set `skill_dir` to the directory containing this `SKILL.md`, then resolve the plugin root as `skill_dir/../..`. Use scripts from `<plugin-root>/scripts/`; do not guess cache paths or copy script bodies into shell commands.
...
Run status checks with enough sandbox access for local loopback requests, LaunchAgent inspection, and macOS code-signing trust evaluation. If the execution environment returns `operation not permitted`, blocks `127.0.0.1`, or prevents Keychain-backed signature validation, label those checks `inconclusive in the current sandbox`; do not report the installed service or signature as failed solely from sandbox-denied probes. Confirm the same checks in the normal host environment before diagnosing a runtime failure.
...
- `accessibility: unavailable`: the running helper itself failed `AXIsProcessTrusted()`. If this is the first stable-signature install, remove the stale entry, add the installed app once in Accessibility settings, and restart the hotkey LaunchAgent. Do not repeatedly reopen System Settings.
...
- Repeated permission loss after rebuild: run `scripts/setup-local-signing.sh` and reinstall. Do not replace that identity with ad-hoc signatures during later updates.
